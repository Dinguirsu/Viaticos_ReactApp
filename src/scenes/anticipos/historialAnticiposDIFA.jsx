...
import { fetchHistorialAnticiposDIFAByYear } from "../../Services/anticiposService";
import { tokens } from "../../theme";
import { dataGridSx } from '../form/datagridStyles';

const ReporteAnticiposAnio = () => {
  ...
  // 🔹 Función que SIEMPRE reemplaza la lista con los datos del año que le pases
  const cargarDatos = async (yearToLoad) => {
    try {
      setLoading(true);
      const resp = await fetchHistorialAnticiposDIFAByYear(yearToLoad);
      ...
      // opcional: si quieres limpiar el buscador cada vez que cambias de año:
      // setSearch("");
    } catch (error) {
      console.error("Error cargando reporte de anticipos por año:", error);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  // Cuando el usuario cambia el año en el select
 ...
 return (
  <Box sx={{ width: "100%", mt: 2 }}>
    ...
    <Box sx={{ mb: 2 }}>
      ...
      >
        Reporte de Anticipos por Año
      </Typography>
      <Typography
        align="center"
        sx={{ mt: 0.5, fontSize: 13, color: "rgba(255,255,255,0.70)" }}
      >
        Filtrá por año y buscá por empleado, área, autorización, lugar o estado.
      </Typography>
    </Box>
    ...
    >
      <CardContent sx={{ p: 2 }}>
        ...
        >
          {/* 🔎 Buscador */}
          <TextField
            label="Buscar (empleado, área, autorización, lugar, estado...)"
            size="small"
            value={search}
        ...
        <Box sx={{ height: 520, width: "100%" }}>
          <DataGrid
            rows={filteredRows}
            columns={columns}
            sx={dataGridSx} // ✅ mismo estilo del dashboard
            loading={loading}
...
export default ReporteAnticiposAnio;
